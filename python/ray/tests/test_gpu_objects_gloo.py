...
def test_trigger_out_of_band_tensordict_transfer(ray_start_regular):
    ...
    ret_val_src = ray.get(src_actor.get_gpu_object.remote(gpu_ref.hex()))
    assert ret_val_src is not None
    print("ret_val_src: ", ret_val_src, flush=True)
    # assert isinstance(ret_val_src, TensorDict)
    # assert torch.equal(ret_val_src["action"], td["action"])
    # assert torch.equal(ret_val_src["reward"], td["reward"])
    ...
    gpu_object_manager.trigger_out_of_band_tensor_transfer(dst_actor, task_args)
    assert ray.get(src_actor.get_gpu_object.remote(gpu_ref.hex())) is None
    meta = gpu_object_manager._get_gpu_object_metadata(gpu_ref)
    print(f"meta: {meta}", flush=True)
    print(f"meta.tensor_meta: {ray.get(meta.tensor_meta)}", flush=True)

    # Check dst_actor has the GPU object
    ret_val_dst = ray.get(dst_actor.get_gpu_object.remote(gpu_ref.hex()))
    assert ret_val_dst is not None
    assert len(ret_val_dst) == 2
    assert torch.equal(ret_val_dst[0], td["action"])
    assert torch.equal(ret_val_dst[1], td["reward"])
    time.sleep(20)
